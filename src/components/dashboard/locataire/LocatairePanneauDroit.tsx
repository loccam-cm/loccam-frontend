...
import {
  IconBell, IconHome2, IconDroplet, IconBolt,
  IconCheck, IconAlertTriangle, IconMessage, IconTool, IconFileText, IconCircleCheck,
...
const NOTIF_STYLES: Record<string, { bg: string; col: string; ico: React.ReactNode }> = {
  ...
  contrat_signe:      { bg: '#F5F3FF', col: '#7C3AED', ico: <IconFileText size={12}/> },
}
...
export function LocatairePanneauDroit({ data, loading, contrat, joursRestants }: Props) {
  return (
    <div className="hidden xl:flex w-72 flex-shrink-0 flex-col overflow-y-auto"
      ...
      <div className="p-5" style={{ background: 'linear-gradient(160deg,#064E3B,#059669)', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
        <div className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: 'rgba(255,255,255,.38)' }}>
        ...
        {loading ? Array(4).fill(0).map((_,i) => (
          <div key={i} className="flex justify-between py-2.5" style={{ borderBottom: '1px solid rgba(255,255,255,.06)' }}>
            <LocataireSkeleton className="h-3 w-28" /><LocataireSkeleton className="h-3 w-20" />
          </div>
        )) : [
          ...
          <div key={r.lbl} className="flex items-center justify-between py-2.5" style={{ borderBottom: '1px solid rgba(255,255,255,.06)' }}>
            ...
            <span className="text-sm font-bold" style={{ color: r.bold ? '#6EE7B7' : '#fff' }}>{r.val}</span>
          </div>
        ))}
      </div>
      ...
      <div className="p-5" style={{ borderBottom: '1px solid #F0FDF4' }}>
          ...
          {[
            ...
            <div key={s.lbl}>
              <div className="flex justify-between text-xs mb-1.5">
                ...
                <span className="font-bold" style={{ color: s.color }}>{loading ? '—' : `${s.val}/${s.max}`}</span>
              </div>
        ...
        <div className="rounded-xl p-3.5" style={{ background: '#F0FDF4', border: '1px solid #D1FAE5' }}>
          ...
          <div className="text-xl font-bold" style={{ color: '#059669' }}>
            {loading ? <LocataireSkeleton className="h-6 w-28" /> : <><AnimatedNumber value={data?.totalPaye ?? 0} /> XAF</>}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-5">
        ...
        {loading ? Array(4).fill(0).map((_,i) => (
          <div key={i} className="flex gap-2.5 py-3" style={{ borderBottom: '1px solid #F8FAFC' }}>
            ...
            <div className="flex-1 space-y-1.5"><LocataireSkeleton className="h-3 w-24" /><LocataireSkeleton className="h-3 w-32" /></div>
          </div>
          ...
          const st = NOTIF_STYLES[n.type] ?? { bg: '#ECFDF5', col: '#059669', ico: <IconBell size={12}/> }
          return (
            <motion.div key={n.id} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: st.bg }}>
