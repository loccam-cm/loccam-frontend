...
import {
  IconFileText, IconPlus, IconSearch, IconArrowLeft,
  IconRefresh, IconX, IconCheck, IconLoader2,
  IconHome2, IconUsers, IconCreditCard, IconCalendar,
  IconChevronRight, IconEdit, IconEye, IconAlertCircle,
  IconFilter, IconDownload, IconCircleCheck,
  IconClockHour4, IconBan, IconSignature,
...
const STATUT_MAP: Record<string, { bg: string; col: string; lbl: string; ico: React.ReactNode }> = {
  actif:      { bg: '#ECFDF5', col: '#059669', lbl: 'Actif',      ico: <IconCircleCheck size={11}/> },
  termine:    { bg: '#F1F5F9', col: '#64748B', lbl: 'Terminé',    ico: <IconBan size={11}/> },
  resilie:    { bg: '#FEF2F2', col: '#DC2626', lbl: 'Résilié',    ico: <IconBan size={11}/> },
  en_attente: { bg: '#FFFBEB', col: '#D97706', lbl: 'En attente', ico: <IconClockHour4 size={11}/> },
  brouillon:  { bg: '#F1F5F9', col: '#94A3B8', lbl: 'Brouillon',  ico: <IconFileText size={11}/> },
}
...
function Avatar({ nom, prenom, size = 36 }: { nom: string; prenom: string; size?: number }) {
  const colors = ['#3B82F6','#059669','#D97706','#7C3AED','#EF4444','#06B6D4']
  const col = colors[(prenom?.charCodeAt(0) ?? 0) % colors.length]
  return (
    <div style={{ width: size, height: size, borderRadius: '10px', background: `${col}18`,
      border: `1.5px solid ${col}30`, display: 'flex', alignItems: 'center',
      justifyContent: 'center', fontWeight: 700, fontSize: size * 0.35, color: col, flexShrink: 0 }}>
      {prenom?.[0]}{nom?.[0]}
    </div>
  )
}
...
export default function ContratsPage() {
  ...
  const emptyForm: FormData = {
    ...
    loyer_mensuel: '', caution: '', jour_echeance: '5', notes: '',
  }
  ...
  const load = async () => {
    ...
    try {
      ...
      const invs = Array.isArray(invRes.data) ? invRes.data : ((invRes.data as any)?.results ?? [])
      invs.filter((i: any) => i.statut === 'acceptee' && i.locataire_id)
        .forEach((i: any) => {
  ...
  const stats = {
    ...
    attente:  contrats.filter(c => c.statut === 'en_attente' || c.statut === 'brouillon').length,
    termines: contrats.filter(c => c.statut === 'termine' || c.statut === 'resilie').length,
  }
      ...
      <div className="flex flex-col min-h-screen"
        ...
        <header className="sticky top-0 z-30 flex items-center gap-3 px-4 sm:px-6 h-14 sm:h-16 bg-white"
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <IconFileText size={17} style={{ color: '#7C3AED', flexShrink: 0 }} />
            <h1 className="text-sm font-bold truncate" style={{ color: '#0F172A' }}>Mes contrats</h1>
            {!loading && (
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: .98 }}
              ...
              style={{ background: 'linear-gradient(135deg,#7C3AED,#6D28D9)', boxShadow: '0 2px 8px rgba(124,58,237,.35)' }}>
              <IconPlus size={15} />
        ...
        <div className="px-4 sm:px-6 pt-4 sm:pt-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            {[
              { lbl: 'Total',      val: stats.total,    col: '#7C3AED', bg: '#F5F3FF', ico: <IconFileText size={15}/> },
              { lbl: 'Actifs',     val: stats.actifs,   col: '#059669', bg: '#ECFDF5', ico: <IconCircleCheck size={15}/> },
              { lbl: 'En attente', val: stats.attente,  col: '#D97706', bg: '#FFFBEB', ico: <IconClockHour4 size={15}/> },
              { lbl: 'Terminés',   val: stats.termines, col: '#64748B', bg: '#F1F5F9', ico: <IconBan size={15}/> },
            ].map((s, i) => (
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            <div className="relative flex-1 min-w-48">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                     ...
                     style={{ width: '100%', height: '42px', padding: '0 12px 0 36px', borderRadius: '10px', border: '1.5px solid #E2E8F0', fontSize: '14px', color: '#0F172A', outline: 'none', background: '#fff', fontFamily: 'inherit' }} />
              {search && (
            ...
            <button onClick={() => setFilterOpen(!filterOpen)}
              ...
              style={{ background: filterStatut !== 'tous' ? '#F5F3FF' : '#fff', border: '1.5px solid', borderColor: filterStatut !== 'tous' ? '#7C3AED' : '#E2E8F0', color: filterStatut !== 'tous' ? '#7C3AED' : '#64748B' }}>
              <IconFilter size={15} />
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                <div className="flex gap-2 flex-wrap pb-1">
                  {[
                    ...
                    <button key={f.val} onClick={() => { setFilter(f.val); setFilterOpen(false) }}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-sm font-bold mb-1" style={{ color: '#0F172A' }}>
                {search || filterStatut !== 'tous' ? 'Aucun contrat trouvé' : 'Aucun contrat'}
              </h3>
              <p className="text-xs mb-4" style={{ color: '#94A3B8' }}>
                {search || filterStatut !== 'tous' ? 'Modifiez vos critères' : 'Créez votre premier contrat de bail'}
              </p>
              {!search && filterStatut === 'tous' && (
                <button onClick={openAdd}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white"
                  style={{ background: 'linear-gradient(135deg,#7C3AED,#6D28D9)' }}>
                  <IconPlus size={14} />Créer un contrat
                </button>
              )}
            </div>
          ) : (
            <>
                ...
                {filtered.map((c, i) => (
                  <motion.div key={c.id}
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      {c.locataire && <Avatar nom={c.locataire.nom} prenom={c.locataire.prenom} size={42} />}
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs truncate" style={{ color: '#64748B' }}>{c.bien?.titre ?? '—'}</span>
                        </div>
                      </div>
              ...
              <div className="hidden sm:block bg-white rounded-2xl overflow-hidden" style={{ border: '1px solid #E2E8F0' }}>
                <div className="grid px-5 py-3"
                     style={{ gridTemplateColumns: '2fr 1.8fr 1fr 1fr 1fr auto', gap: '12px', background: '#F8FAFC', borderBottom: '1px solid #F1F5F9' }}>
                  {['Locataire', 'Logement', 'Loyer', 'Début', 'Statut', ''].map(h => (
                ...
                {filtered.map((c, i) => (
                  <motion.div key={c.id}
                    ...
                    style={{ gridTemplateColumns: '2fr 1.8fr 1fr 1fr 1fr auto', gap: '12px', borderBottom: '1px solid #F8FAFC' }}
                    onClick={() => setSelected(c)}>

                    <div className="flex items-center gap-3 min-w-0">
                      {c.locataire && <Avatar nom={c.locataire.nom} prenom={c.locataire.prenom} />}
                      <div className="min-w-0">
                        <div className="text-sm font-semibold truncate" style={{ color: '#0F172A' }}>{c.locataire?.nom_complet ?? '—'}</div>
                        <div className="text-xs truncate" style={{ color: '#94A3B8' }}>{c.locataire?.email ?? '—'}</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-1.5 min-w-0">
                      <IconHome2 size={13} style={{ color: '#94A3B8', flexShrink: 0 }} />
                      <span className="text-sm truncate" style={{ color: '#475569' }}>{c.bien?.titre ?? '—'}</span>
                    </div>
                    ...
                    <div className="text-sm" style={{ color: '#475569' }}>
                      {c.date_debut ? new Date(c.date_debut).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: '2-digit' }) : '—'}
                    </div>
          ...
          {showForm && (
            <>
              ...
              <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                ...
                style={{ width: 'min(520px,100vw)', background: '#fff', boxShadow: '-8px 0 40px rgba(0,0,0,.15)' }}>

                <div className="flex items-center justify-between px-6 py-5 sticky top-0 bg-white z-10"
                     style={{ borderBottom: '1px solid #F1F5F9' }}>
                  <div>
                    <h2 className="text-base font-bold" style={{ color: '#0F172A' }}>
                      {editItem ? 'Modifier le contrat' : 'Nouveau contrat de bail'}
                    </h2>
                    <p className="text-xs mt-0.5" style={{ color: '#94A3B8' }}>
                      {editItem ? 'Modification en cours' : 'Associez un locataire à un logement'}
                    </p>
                  </div>
                ...
                <div className="px-6 py-5 space-y-5">
                  {/* Bien */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#94A3B8' }}>Logement</div>
                    <div>
                  ...
                  {/* Locataire */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#94A3B8' }}>Locataire</div>
                    <div>
                      <label className="block text-xs font-semibold mb-1.5" style={{ color: '#374151' }}>
                        Locataire <span style={{ color: '#EF4444' }}>*</span>
                      </label>
                      <select value={form.locataire} onChange={e => set('locataire', e.target.value)}
                              className={`sfield ${errors.locataire ? 'err' : ''}`}>
                        <option value="">-- Choisir un locataire --</option>
                        {locataires.map(l => (
                          <option key={l.id} value={l.id}>{l.nom_complet} ({l.email})</option>
                        ))}
                      </select>
                      {errors.locataire && <p className="text-xs mt-1" style={{ color: '#EF4444' }}>{errors.locataire}</p>}
                      {locataires.length === 0 && (
                        <p className="text-xs mt-1.5 flex items-center gap-1" style={{ color: '#D97706' }}>
                          <IconAlertCircle size={12} />Aucun locataire — invitez d&apos;abord un locataire
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Dates */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#94A3B8' }}>Durée du bail</div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        {errors.date_debut && <p className="text-xs mt-1" style={{ color: '#EF4444' }}>{errors.date_debut}</p>}
                      </div>
                      <div>
                        <label className="block text-xs font-semibold mb-1.5" style={{ color: '#374151' }}>
                          Date de fin <span style={{ color: '#94A3B8' }}>(optionnel)</span>
                        </label>
                  ...
                  {/* Finances */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#94A3B8' }}>Conditions financières</div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-semibold mb-1.5" style={{ color: '#374151' }}>
                          Loyer mensuel (XAF) <span style={{ color: '#EF4444' }}>*</span>
                        </label>
                        <input type="number" value={form.loyer_mensuel} onChange={e => set('loyer_mensuel', e.target.value)}
                               placeholder="85000" className={`ifield ${errors.loyer_mensuel ? 'err' : ''}`} />
                        {errors.loyer_mensuel && <p className="text-xs mt-1" style={{ color: '#EF4444' }}>{errors.loyer_mensuel}</p>}
                      </div>
                      <div>
                        <label className="block text-xs font-semibold mb-1.5" style={{ color: '#374151' }}>Caution (XAF)</label>
                        <input type="number" value={form.caution} onChange={e => set('caution', e.target.value)}
                               placeholder="85000" className="ifield" />
                      </div>
                    </div>
                    <div className="mt-3">
                      <label className="block text-xs font-semibold mb-1.5" style={{ color: '#374151' }}>
                      ...
                      <select value={form.jour_echeance} onChange={e => set('jour_echeance', e.target.value)} className="sfield">
                  ...
                  {/* Erreurs API */}
                  {Object.keys(errors).filter(k => !['bien','locataire','date_debut','loyer_mensuel'].includes(k)).length > 0 && (
                    <div className="flex items-start gap-2 p-3 rounded-xl"
                         style={{ background: '#FEF2F2', border: '1px solid #FECACA' }}>
                      <IconAlertCircle size={14} style={{ color: '#EF4444', flexShrink: 0, marginTop: '1px' }} />
                      <div className="text-xs" style={{ color: '#DC2626' }}>
                        {Object.entries(errors)
                          .filter(([k]) => !['bien','locataire','date_debut','loyer_mensuel'].includes(k))
                  ...
                  {/* Notes */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#94A3B8' }}>Notes</div>
                ...
                <div className="sticky bottom-0 bg-white px-6 py-4 flex gap-3"
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: .98 }}
                    ...
                    style={{ background: success ? '#059669' : 'linear-gradient(135deg,#7C3AED,#6D28D9)', boxShadow: '0 2px 10px rgba(124,58,237,.3)' }}>
                    {saving
                      ? <><IconLoader2 size={15} style={{ animation: 'spin 1s linear infinite' }} />Enregistrement...</>
                      : success
                        ? <><IconCheck size={15} />{editItem ? 'Mis à jour !' : 'Contrat créé !'}</>
                        : <><IconCheck size={15} />{editItem ? 'Mettre à jour' : 'Créer le contrat'}</>}
                  </motion.button>
                </div>
              </motion.div>
            </>
            ...
            <>
              ...
              <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                ...
                <div className="flex items-center justify-between px-5 py-4 sticky top-0 bg-white z-10"
                     style={{ borderBottom: '1px solid #F1F5F9' }}>
                  <h2 className="text-sm font-bold" style={{ color: '#0F172A' }}>Détail du contrat</h2>
                  <div className="flex items-center gap-2">
                ...
                <div className="px-5 py-5 space-y-5">
                  ...
                  <div className="p-4 rounded-2xl"
                       style={{ background: selected.statut === 'actif' ? '#ECFDF5' : selected.statut === 'brouillon' ? '#FFFBEB' : '#F8FAFC',
                                border: `1px solid ${selected.statut === 'actif' ? '#A7F3D0' : selected.statut === 'brouillon' ? '#FDE68A' : '#E2E8F0'}` }}>
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Statut</div>
                        <StatutBadge statut={selected.statut} />
                      </div>
                      <div className="text-right">
                        <div className="text-xs mb-1" style={{ color: '#94A3B8' }}>Référence</div>
                        <div className="text-sm font-bold" style={{ color: '#0F172A' }}>#{selected.id}</div>
                      </div>
                    </div>
                    {/* Signatures */}
                    <div className="grid grid-cols-2 gap-2 pt-3" style={{ borderTop: '1px solid rgba(0,0,0,.06)' }}>
                      {[
                        { lbl: 'Bailleur', signed: selected.signe_bailleur },
                        { lbl: 'Locataire', signed: selected.signe_locataire },
                      ].map(s => (
                        <div key={s.lbl} className="flex items-center gap-2 p-2 rounded-lg"
                             style={{ background: s.signed ? '#ECFDF5' : '#F8FAFC' }}>
                          <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0"
                               style={{ background: s.signed ? '#059669' : '#E2E8F0' }}>
                            {s.signed
                              ? <IconCheck size={12} color="white" />
                              : <IconClockHour4 size={12} style={{ color: '#94A3B8' }} />}
                          </div>
                  ...
                  {!selected.signe_bailleur && (
                    <motion.button
                      ...
                      style={{ background: signing ? '#94A3B8' : 'linear-gradient(135deg,#7C3AED,#6D28D9)', boxShadow: signing ? 'none' : '0 3px 12px rgba(124,58,237,.4)' }}>
                      {signing
                        ? <><IconLoader2 size={15} style={{ animation: 'spin 1s linear infinite' }} />Signature...</>
                        : <><IconSignature size={15} />Signer le contrat (bailleur)</>}
                    </motion.button>
                  )}
                  ...
                  {selected.bien && (
                    <div>
                      ...
                      <div className="flex items-center gap-3 p-4 rounded-2xl"
                        ...
                        <div>
                          <div className="text-white font-bold text-sm">{selected.bien.titre}</div>
                          <div className="text-xs" style={{ color: 'rgba(255,255,255,.5)' }}>{selected.bien.adresse}</div>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {selected.locataire && (
                    <div>
                      ...
                      <div className="flex items-center gap-3 p-3 rounded-xl"
                        ...
                        <Avatar nom={selected.locataire.nom} prenom={selected.locataire.prenom} size={40} />
                        <div>
                          <div className="text-sm font-bold" style={{ color: '#0F172A' }}>{selected.locataire.nom_complet}</div>
                  ...
                  {/* Conditions */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#94A3B8' }}>Conditions</div>
                    <div className="space-y-2">
                      {[
                        { lbl: 'Loyer mensuel',  val: selected.loyer_mensuel ? `${selected.loyer_mensuel.toLocaleString('fr-FR')} XAF` : '—', col: '#059669' },
                        { lbl: 'Caution',        val: selected.caution ? `${selected.caution.toLocaleString('fr-FR')} XAF` : '—', col: '#0F172A' },
                        { lbl: "Date d'entrée",  val: selected.date_debut ? new Date(selected.date_debut).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) : '—', col: '#0F172A' },
                        { lbl: 'Date de fin',    val: selected.date_fin ? new Date(selected.date_fin).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Bail à durée indéterminée', col: '#64748B' },
                      ].map(row => (
                        <div key={row.lbl} className="flex items-center justify-between py-2.5 px-3 rounded-xl"
                             style={{ background: '#F8FAFC' }}>
                          <span className="text-xs" style={{ color: '#64748B' }}>{row.lbl}</span>
                          <span className="text-sm font-semibold" style={{ color: row.col }}>{row.val}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { ico: <IconDownload size={15}/>,   lbl: 'Télécharger PDF',    col: '#7C3AED', bg: '#F5F3FF' },
                        { ico: <IconCreditCard size={15}/>, lbl: 'Voir paiements',     col: '#059669', bg: '#ECFDF5' },
                        { ico: <IconUsers size={15}/>,      lbl: 'Contacter locataire', col: '#2563EB', bg: '#EFF6FF' },
                        { ico: <IconBan size={15}/>,        lbl: 'Résilier contrat',   col: '#DC2626', bg: '#FEF2F2' },
                      ].map(a => (
                        <button key={a.lbl}
                  ...
                  {(selected as any).notes && (
                    <div>
                      <div className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#94A3B8' }}>Notes</div>
