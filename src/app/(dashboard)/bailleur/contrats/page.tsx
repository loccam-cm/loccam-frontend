...
interface FormData {
  bien: string;
  locataire: string; // ← ID du locataire (integer)
  date_debut: string;
  date_fin: string;
...
export default function ContratsPage() {
  ...
  const load = async () => {
    ...
    try {
      const [cRes, bRes, invRes] = await Promise.all([
        ...
        // Locataires depuis invitations acceptées
        api.get<any[]>("/invitations/").catch(() => ({ data: [] })),
      ]);
      setContrats(cRes.data.results);
      setBiens(bRes.data.results);

      // Extraire les locataires uniques depuis invitations acceptées + contrats existants
      const locSet = new Map<number, LocataireOption>();

      // Depuis les invitations acceptées
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      // ✅ Payload correct — locataire en ID entier
      const payload = {
                  ...
                  <div>
                    ...
                    <div>
                      ...
                      {locataires.length === 0 && (
                        ...
                        >
                          <IconAlertCircle size={12} />
                          Aucun locataire disponible — invitez d&apos;abord un
                          locataire
                        </p>
                      )}
                    </div>
              ...
              >
                ...
                <div className="px-5 py-5 space-y-5">
                  ...
                  <div
                    ...
                    style={{
                      background:
                        selected.statut === "actif" ? "#ECFDF5" : "#F8FAFC",
                      border: `1px solid ${selected.statut === "actif" ? "#A7F3D0" : "#E2E8F0"}`,
                    }}
                  >
                    <div>
                      <div
