...
import PlanGate, { BanniereUpgrade } from '@/components/plan/PlanGate'
import {
  ...
  IconBuildingBank, IconLoader2, IconLock,
...
export default function PaiementsPage() {
  ...
  // Droits selon le plan
  const peutVoirAnalytique = peut('analytique')   // Pro+
  const peutQuittances     = peut('quittances')   // Pro+
  const peutExporter       = peut('export')       // Business
  ...
  const telechargerQuittance = async (e: React.MouseEvent, paiementId: number) => {
    e.stopPropagation()
    if (!peutQuittances) return        // sécurité côté client
      ...
      <div className="flex flex-col min-h-screen"
        ...
        <header className="sticky top-0 z-30 flex items-center gap-3 px-4 sm:px-6 h-14 sm:h-16 bg-white"
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Export CSV — réservé Business ── */}
            <PlanGate fonctionnalite="export">
              {({ autorise, ouvrir }) => (
                <button
                  onClick={autorise ? undefined : ouvrir}
                  className="hidden sm:flex items-center gap-2 px-3 h-9 rounded-xl text-xs font-semibold"
                  style={{ background: autorise ? '#ECFDF5' : '#F1F5F9', color: autorise ? '#059669' : '#94A3B8', border:`1px solid ${autorise ? '#A7F3D0' : '#E2E8F0'}`, cursor:'pointer' }}>
                  {autorise ? <IconDownload size={14}/> : <IconLock size={13}/>}
                  Exporter CSV
                  {!autorise && <span style={{ fontSize:'10px', fontWeight:700, padding:'1px 6px', borderRadius:'100px', background:'rgba(124,58,237,.12)', color:'#7C3AED' }}>Business</span>}
                </button>
              )}
            </PlanGate>
          </div>
        ...
        <div className="px-4 sm:px-6 pt-4 sm:pt-5 space-y-4">
          ...
          {/* Bandeau quittances si plan gratuit */}
          <BanniereUpgrade fonctionnalite="quittances"/>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              <motion.div key={s.lbl} initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
          ...
          {/* ── GRAPHIQUE MENSUEL — réservé Pro+ ── */}
          {peutVoirAnalytique ? (
            <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }}
              transition={{ delay:.15 }} className="bg-white rounded-2xl p-4 sm:p-5"
              style={{ border:'1px solid #E2E8F0' }}>
                ...
                {chartData.map((d, i) => (
                  <div key={d.mois} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <span className="text-center"
                          style={{ fontSize:'9px', color: d.actif ? '#059669' : '#94A3B8', fontWeight: d.actif ? 700 : 400 }}>
                      {d.mois}
                    </span>
                  </div>
                ))}
            ...
            /* Graphique verrouillé — plan gratuit */
            <div className="bg-white rounded-2xl p-4 sm:p-5 relative overflow-hidden"
                 style={{ border:'1px solid #E2E8F0' }}>
              {/* Faux graphique flou */}
              <div style={{ filter:'blur(4px)', opacity:.3, pointerEvents:'none', userSelect:'none' }}>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-0.5" style={{ color:'#94A3B8' }}>Revenus mensuels</div>
                    <div className="text-lg font-bold" style={{ color:'#0F172A' }}>— XAF</div>
                  </div>
                </div>
                <div className="flex items-end gap-1 sm:gap-2" style={{ height:'72px' }}>
                  {MOIS.map((m, i) => (
                    <div key={m} className="flex-1 flex flex-col items-center gap-1">
                      <div className="w-full rounded-t" style={{ height:`${20 + Math.random() * 60}%`, background:'#E2E8F0', minHeight:'4px' }}/>
              ...
              <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'10px' }}>
                <div style={{ width:'40px', height:'40px', borderRadius:'14px', background:'linear-gradient(135deg,rgba(124,58,237,.15),rgba(37,99,235,.15))', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <IconLock size={18} style={{ color:'#7C3AED' }}/>
                </div>
                <div style={{ textAlign:'center' }}>
                  <div style={{ fontSize:'13px', fontWeight:700, color:'#0F172A', marginBottom:'2px' }}>Analytique disponible en Pro</div>
                  <div style={{ fontSize:'12px', color:'#64748B' }}>Suivez vos revenus mois par mois</div>
                </div>
                <Link href="/bailleur/abonnement"
                      style={{ padding:'8px 20px', borderRadius:'12px', background:'linear-gradient(135deg,#7C3AED,#2563EB)', color:'#fff', textDecoration:'none', fontSize:'12px', fontWeight:700 }}>
                  Passer au Pro
                </Link>
              </div>
            </div>
          ...
          <div className="flex gap-2 flex-wrap">
            <div className="relative flex-1 min-w-48">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                     ...
                     style={{ width:'100%', height:'42px', padding:'0 36px', borderRadius:'10px', border:'1.5px solid #E2E8F0', fontSize:'14px', color:'#0F172A', outline:'none', background:'#fff', fontFamily:'inherit', transition:'border-color .15s', boxSizing:'border-box' }}
            ...
            <button onClick={() => setFilterOpen(!filterOpen)}
                    ...
                    style={{ background: filterStatut !== 'tous' ? '#ECFDF5' : '#fff', border:'1.5px solid', borderColor: filterStatut !== 'tous' ? '#059669' : '#E2E8F0', color: filterStatut !== 'tous' ? '#059669' : '#64748B', cursor:'pointer' }}>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-xs" style={{ color:'#94A3B8' }}>
                {search || filterStatut !== 'tous' || filterMois !== 'tous' ? 'Modifiez vos critères' : 'Les paiements apparaîtront ici'}
              </p>
            </div>
          ) : (
            <>
                  ...
                  <div key={periode}>
                    ...
                    {items.map((p, i) => (
                      <motion.div key={p.id}
                        ...
                        {/* Quittance mobile — Pro+ uniquement */}
                        {p.statut === 'confirme' && (
                          peutQuittances ? (
                            <button
                              ...
                              style={{ background:'#ECFDF5', color:'#059669', border:'1px solid #A7F3D0', cursor:'pointer', opacity: downloading === p.id ? 0.7 : 1 }}>
                              {downloading === p.id
                            ...
                            <Link href="/bailleur/abonnement"
                              ...
                              <IconLock size={12}/>Quittance — Plan Pro
                            </Link>
                          )
                        )}
                      </motion.div>
                    ...
                    {items.map((p, i) => (
                      <motion.div key={p.id}
                        ...
                        <div className="flex gap-1.5">
                          ...
                          {/* Quittance desktop — Pro+ uniquement */}
                          {p.statut === 'confirme' && (
                            peutQuittances ? (
                              <button onClick={e => telechargerQuittance(e, p.id)}
                                      disabled={downloading === p.id}
                                      title="Télécharger la quittance"
                                      className="w-8 h-8 rounded-lg flex items-center justify-center"
                                      style={{ background:'#ECFDF5', color:'#059669', border:'none', cursor:'pointer', opacity: downloading === p.id ? 0.6 : 1 }}>
                                {downloading === p.id
                              ...
                              <Link href="/bailleur/abonnement"
                                    onClick={e => e.stopPropagation()}
                                    title="Quittances disponibles en Plan Pro"
                                    className="w-8 h-8 rounded-lg flex items-center justify-center"
            ...
            <>
              ...
              <motion.div initial={{ x:'100%' }} animate={{ x:0 }} exit={{ x:'100%' }}
                ...
                <div className="px-5 py-5 space-y-5">
                  ...
                  {/* Bouton quittance drawer — Pro+ */}
                  {selected.statut === 'confirme' && (
                    peutQuittances ? (
                      <button onClick={e => telechargerQuittance(e, selected.id)}
                              disabled={downloading === selected.id}
                              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold"
                              style={{ background:'#ECFDF5', color:'#059669', border:'1px solid #A7F3D0', cursor:'pointer', opacity: downloading === selected.id ? 0.7 : 1 }}>
                      ...
                      <Link href="/bailleur/abonnement"
                            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold"
                            style={{ background:'linear-gradient(135deg,#7C3AED,#2563EB)', color:'#fff', textDecoration:'none' }}>
                        <IconLock size={15}/>Débloquer les quittances — Plan Pro
                      </Link>
                    )
                  )}
                </div>
