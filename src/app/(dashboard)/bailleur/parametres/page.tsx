...
export default function BailleurParametresPage() {
  const { user } = useAuth();
  ...
  const sauvegarder = async () => {
    ...
    try {
      ...
      // ✅ Si la langue a changé, l'appliquer et recharger
          ...
          <div className="flex items-center gap-2 flex-1">
            ...
            <div>
              ...
              <p
                ...
                style={{ color: "#94A3B8" }}
              >
                Notifications, paiements, affichage
              </p>
            </div>
          </div>
          <motion.button
            ...
            prefs && (
              <>
                ...
                <Section titre="Notifications" ico={<IconBell size={15} />}>
                  <Row lbl="Paiement confirmé" desc="Email à chaque loyer reçu">
                    <Toggle
                  ...
                  <Row
                    lbl="Impayé détecté"
                    desc="Email si un loyer est en retard"
                  >
                    <Toggle
                      value={prefs.notif_email_impayes}
                      onChange={(v) => set("notif_email_impayes", v)}
                    />
                  </Row>
                  <Row
                    lbl="Nouveau signalement"
                    desc="Email quand un locataire signale une panne"
                  >
                    <Toggle
                      value={prefs.notif_email_signalements}
                      onChange={(v) => set("notif_email_signalements", v)}
                    />
                  </Row>
                  <Row
                    lbl="Notifications in-app"
                    desc="Alertes dans le tableau de bord"
                  >
                    <Toggle
                ...
                <Section titre="Paiements" ico={<IconCreditCard size={15} />}>
                  <Row
                    ...
                    desc="Utilisé par défaut pour vos locataires"
                  >
                    <select
                ...
                <Section titre="Affichage" ico={<IconWorld size={15} />}>
                  <Row lbl="Langue de l'interface">
                    {/* ✅ prefs.langue pour l'affichage, changerLangue au moment de sauvegarder */}
                    <select
                      value={prefs.langue}
                  ...
                  <Row
                    lbl="Devise"
                    desc="Toutes les valeurs sont affichées en XAF"
                  >
                    <span
                ...
                {/* ── CONFIDENTIALITÉ ───────────────────────── */}
                <Section titre="Confidentialité" ico={<IconShield size={15} />}>
                  <Row
                    lbl="Profil visible"
                    desc="Vos locataires peuvent voir votre profil"
                  >
                    <Toggle
                      value={prefs.profil_visible}
                      onChange={(v) => set("profil_visible", v)}
                    />
                  </Row>
                </Section>
