...
import {
  IconHome2, IconPlus, IconSearch, IconFilter,
  IconBuilding, IconMapPin, IconCurrencyDollar,
  IconEye, IconEdit, IconTrash, IconX, IconCheck,
  IconLoader2, IconPhoto, IconChevronRight,
  IconLayoutGrid, IconList, IconBed, IconDoor,
...
export default function BiensPage() {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    structure: '', meuble: false, eau_incluse: false, elec_incluse: false,
  })
  ...
  const openAdd = () => {
    ...
    setForm({
      ...
      prix: '', caution: '', type_bien: 'studio', categorie: 'residentiel',
      surface: '', nb_pieces: '', nb_chambres: '', etage: '',
      structure: structures[0]?.id.toString() ?? '', meuble: false,
      eau_incluse: false, elec_incluse: false,
    })
    setErrors({})
    setShowForm(true)
  }
  ...
  const openEdit = (b: Bien) => {
    ...
    setForm({
      ...
      adresse: b.adresse, ville: b.ville ?? 'Douala',
      prix: b.prix.toString(), caution: b.caution?.toString() ?? '',
      type_bien: b.type_bien, categorie: b.categorie,
      surface: b.surface?.toString() ?? '', nb_pieces: b.nb_pieces?.toString() ?? '',
      nb_chambres: b.nb_chambres?.toString() ?? '', etage: b.etage?.toString() ?? '',
      structure: b.structure?.toString() ?? '', meuble: b.meuble ?? false,
      eau_incluse: b.eau_incluse ?? false, elec_incluse: b.elec_incluse ?? false,
    })
    setErrors({})
    setShowForm(true)
  }
  ...
  const filtered = biens.filter(b => {
    const matchSearch = !search || b.titre.toLowerCase().includes(search.toLowerCase()) || b.adresse.toLowerCase().includes(search.toLowerCase())
                ...
                {filtered.map((b, i) => (
                  <motion.div key={b.id}
                    ...
                    <div className="relative h-44 flex items-center justify-center"
                      ...
                      <IconBuilding size={40} style={{ color: '#93C5FD' }} />
                ...
                <div className="px-6 py-5 space-y-5">
                  ...
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3"
                         style={{ color: '#94A3B8' }}>Informations générales</div>
                      ...
                      <div>
                        ...
                        {structures.length === 0 && (
                          <p className="text-xs mt-1.5 flex items-center gap-1" style={{ color: '#D97706' }}>
                            ...
                            Créez d&apos;abord une structure dans &ldquo;Structures&rdquo;
                          </p>
                        )}
                      </div>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={o.key} className="flex items-center justify-between py-3 px-4 rounded-xl"
                          ...
                          <button
                            className="toggle"
                            onClick={() => set(o.key as keyof FormData, !form[o.key as keyof FormData])}
                            style={{
                              background: form[o.key as keyof FormData] ? '#2563EB' : '#E2E8F0',
                            }}>
                            <span style={{ position: 'absolute', top: '3px', left: form[o.key as keyof FormData] ? '21px' : '3px', width: '16px', height: '16px', borderRadius: '50%', background: '#fff', transition: 'left .2s', boxShadow: '0 1px 4px rgba(0,0,0,.2)' }} />
                          </button>
                        </div>
                      ))}
                    </div>
