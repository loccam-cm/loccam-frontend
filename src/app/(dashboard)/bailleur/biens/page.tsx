...
import {
  IconHome2, IconPlus, IconSearch, IconFilter, IconBuilding,
  IconMapPin, IconEye, IconEdit, IconTrash, IconX, IconCheck,
  IconLoader2, IconPhoto, IconLayoutGrid, IconList,
  IconAlertCircle, IconRefresh, IconArrowLeft,
} from "@tabler/icons-react";

interface Structure { id: number; nom: string; }

interface FormData {
  titre: string; description: string; adresse: string; ville: string;
  prix: string; caution: string; type_bien: string; categorie: string;
  surface: string; nb_pieces: string; nb_chambres: string; etage: string;
  structure: string; meuble: boolean; eau_incluse: boolean; elec_incluse: boolean;
}

const TYPES_BIEN = [
  { val: "studio", lbl: "Studio" }, { val: "f1", lbl: "F1" },
  { val: "f2", lbl: "F2" }, { val: "f3", lbl: "F3" }, { val: "f4", lbl: "F4+" },
  { val: "duplex", lbl: "Duplex" }, { val: "villa", lbl: "Villa" },
  { val: "boutique", lbl: "Boutique" }, { val: "bureau", lbl: "Bureau" },
  { val: "magasin", lbl: "Magasin" }, { val: "entrepot", lbl: "Entrepôt" },
];
...
function BiensPageContent() {
  ...
  const [form, setForm] = useState<FormData>({
    titre: "", description: "", adresse: "", ville: "Douala",
    prix: "", caution: "", type_bien: "studio", categorie: "residentiel",
    surface: "", nb_pieces: "", nb_chambres: "", etage: "", structure: "",
    meuble: false, eau_incluse: false, elec_incluse: false,
  });
  ...
  const load = async () => {
    ...
    try {
      ...
      setBiens(biensRes.data.results.map((b: Bien) => ({
        ...b,
        photos: photosData.filter((d: { object_id: number }) => d.object_id === b.id),
      })));
  ...
  const openAdd = () => {
    setEditBien(null);
    setForm({
      titre: "", description: "", adresse: "", ville: "Douala", prix: "", caution: "",
      type_bien: "studio", categorie: "residentiel", surface: "", nb_pieces: "",
      nb_chambres: "", etage: "", structure: structures[0]?.id.toString() ?? "",
      meuble: false, eau_incluse: false, elec_incluse: false,
    });
  ...
  const openEdit = (b: Bien) => {
    setEditBien(b);
    setForm({
      titre: b.titre, description: b.description ?? "", adresse: b.adresse,
      ville: b.ville ?? "Douala", prix: b.prix.toString(),
      caution: b.caution?.toString() ?? "", type_bien: b.type_bien,
      categorie: b.categorie, surface: b.surface?.toString() ?? "",
      nb_pieces: b.nb_pieces?.toString() ?? "", nb_chambres: b.nb_chambres?.toString() ?? "",
      etage: b.etage?.toString() ?? "", structure: b.structure?.toString() ?? "",
      meuble: b.meuble ?? false, eau_incluse: b.eau_incluse ?? false,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editBien) { await api.put(`/biens/${editBien.id}/`, payload); }
      else { await api.post("/biens/", payload); }
      setSuccess(true);
      setTimeout(() => { setSuccess(false); setShowForm(false); load(); }, 1500);
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await api.delete(`/biens/${id}/`); load(); } catch { }
  };
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <header className="flex items-center gap-4 px-6 h-16 flex-shrink-0 bg-white"
            ...
            <div className="flex items-center gap-2 flex-1">
              ...
              <h1 className="text-sm font-bold" style={{ color: "#0F172A" }}>Mes biens</h1>
              {!loading && (
          ...
          {/* Toolbar */}
          <div className="flex items-center gap-3 px-6 py-4 flex-shrink-0 flex-wrap gap-y-2"
            ...
            <div className="relative flex-1 min-w-48">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)}
                placeholder="Rechercher un bien..." className="input-field" style={{ paddingLeft: "36px" }} />
            </div>
            <div className="flex gap-1 p-1 rounded-xl" style={{ background: "#F1F5F9" }}>
              {[{ val: "tous", lbl: "Tous" }, { val: "libre", lbl: "Libres" }, { val: "occupe", lbl: "Occupés" }].map((f) => (
            ...
            <div className="flex gap-1 p-1 rounded-xl" style={{ background: "#F1F5F9" }}>
              <button onClick={() => setView("grid")} className="w-8 h-8 rounded-lg flex items-center justify-center transition-all"
                style={view === "grid" ? { background: "#fff", boxShadow: "0 1px 3px rgba(0,0,0,.1)" } : {}}>
                <IconLayoutGrid size={15} style={{ color: view === "grid" ? "#2563EB" : "#94A3B8" }} />
              </button>
              <button onClick={() => setView("list")} className="w-8 h-8 rounded-lg flex items-center justify-center transition-all"
                style={view === "list" ? { background: "#fff", boxShadow: "0 1px 3px rgba(0,0,0,.1)" } : {}}>
                <IconList size={15} style={{ color: view === "list" ? "#2563EB" : "#94A3B8" }} />
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {loading ? (
              <div className={view === "grid" ? "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4" : "flex flex-col gap-3"}>
                {Array(6).fill(0).map((_, i) => <Skeleton key={i} className={view === "grid" ? "h-64" : "h-24"} />)}
              </div>
            ) : filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-base font-bold mb-2" style={{ color: "#0F172A" }}>
                  {search || filterStatut !== "tous" ? "Aucun bien trouvé" : "Aucun bien pour l'instant"}
                </h3>
                <p className="text-sm mb-5" style={{ color: "#64748B" }}>
                  {search || filterStatut !== "tous" ? "Modifiez vos filtres de recherche" : "Ajoutez votre premier bien pour commencer"}
                </p>
                  ...
                  <motion.div key={b.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="relative h-44 overflow-hidden"
                      ...
                      {b.photos && b.photos.length > 0 ? (
                        <img src={b.photos.find((p) => p.est_principal)?.url_publique ?? b.photos[0].url_publique}
                          alt={b.titre} className="w-full h-full object-cover"
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                          ...
                          <span className="text-xs font-medium" style={{ color: "#93C5FD" }}>Aucune photo</span>
                        </div>
                      )}
                      {b.photos && b.photos.length > 0 && (
                        <div className="absolute bottom-2 right-2 flex items-center gap-1 px-2 py-1 rounded-lg"
                          ...
                          <span className="text-xs font-semibold text-white">{b.photos.length}</span>
                        </div>
                      )}
                      <div className="absolute top-3 right-3"><StatutBadge statut={b.statut} /></div>
                    ...
                    <div className="p-4">
                      <h3 className="text-sm font-bold mb-1 truncate" style={{ color: "#0F172A" }}>{b.titre}</h3>
                      <div className="flex items-center gap-1.5 mb-3">
                        <IconMapPin size={12} style={{ color: "#94A3B8", flexShrink: 0 }} />
                        <span className="text-xs truncate" style={{ color: "#64748B" }}>{b.adresse}</span>
                      </div>
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <div className="text-xs mb-0.5" style={{ color: "#94A3B8" }}>Loyer mensuel</div>
                          <div className="text-base font-bold" style={{ color: "#059669" }}>{b.prix.toLocaleString("fr-FR")} XAF</div>
                        </div>
                        {b.surface && (
                          <div className="text-right">
                            <div className="text-xs mb-0.5" style={{ color: "#94A3B8" }}>Surface</div>
                            <div className="text-sm font-semibold" style={{ color: "#0F172A" }}>{b.surface} m²</div>
                          </div>
                        )}
                      </div>
                      <div className="flex gap-1.5 pt-3" style={{ borderTop: "1px solid #F1F5F9" }}>
                        ...
                        <button onClick={() => setGalerieBien(b)}
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold"
                  ...
                  <motion.div key={b.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <h3 className="text-sm font-bold truncate" style={{ color: "#0F172A" }}>{b.titre}</h3>
                    ...
                    <div className="flex-shrink-0 text-right hidden sm:block">
                      <div className="text-xs mb-0.5" style={{ color: "#94A3B8" }}>Loyer</div>
                      <div className="text-sm font-bold" style={{ color: "#059669" }}>{b.prix.toLocaleString("fr-FR")} XAF</div>
                    </div>
            ...
            <>
              ...
              <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
                ...
                <div className="flex items-center justify-between px-6 py-5 sticky top-0 bg-white z-10"
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: "#94A3B8" }}>
                      {editBien ? `Modification de "${editBien.titre}"` : "Remplissez les informations du logement"}
                    </p>
                  </div>
                ...
                <div className="px-6 py-5 space-y-5">
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>
                          Titre du bien <span style={{ color: "#EF4444" }}>*</span>
                        </label>
                        ...
                        {errors.titre && <p className="text-xs mt-1" style={{ color: "#EF4444" }}>{errors.titre}</p>}
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>Type de bien</label>
                          <select value={form.type_bien} onChange={(e) => set("type_bien", e.target.value)} className="select-field">
                            {TYPES_BIEN.map((t) => <option key={t.val} value={t.val}>{t.lbl}</option>)}
                          </select>
                        </div>
                        <div>
                          <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>Catégorie</label>
                          <select value={form.categorie} onChange={(e) => set("categorie", e.target.value)} className="select-field">
                            {CATEGORIES.map((c) => <option key={c.val} value={c.val}>{c.lbl}</option>)}
                          </select>
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>
                          Structure / Immeuble <span style={{ color: "#EF4444" }}>*</span>
                        </label>
                        ...
                        {errors.structure && <p className="text-xs mt-1" style={{ color: "#EF4444" }}>{errors.structure}</p>}
                        {structures.length === 0 && (
                          <p className="text-xs mt-1.5 flex items-center gap-1" style={{ color: "#D97706" }}>
                            <IconAlertCircle size={12} /> Créez d&apos;abord une structure dans &ldquo;Structures&rdquo;
                          </p>
                        )}
                      </div>
                      <div>
                        ...
                        <textarea value={form.description} onChange={(e) => set("description", e.target.value)}
                          placeholder="Décrivez le bien..." className="textarea-field" />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Finances */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "#94A3B8" }}>Finances</div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-semibold mb-1.5" style={{ color: "#374151" }}>
                        ...
                        <input type="number" value={form.prix} onChange={(e) => set("prix", e.target.value)}
                          placeholder="85000" className={`input-field ${errors.prix ? "error" : ""}`} />
                        {errors.prix && <p className="text-xs mt-1" style={{ color: "#EF4444" }}>{errors.prix}</p>}
                      </div>
                      <div>
                  ...
                  {/* Caractéristiques */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "#94A3B8" }}>Caractéristiques</div>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        <div key={f.key}>
                          ...
                          <input type="number" value={form[f.key as keyof FormData] as string}
                  ...
                  {/* Options */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "#94A3B8" }}>Options incluses</div>
                    <div className="space-y-3">
                      {[
                        { key: "meuble", lbl: "Bien meublé", sub: "Le logement est fourni avec des meubles" },
                        { key: "eau_incluse", lbl: "Eau incluse", sub: "Charges eau comprises dans le loyer" },
                        { key: "elec_incluse", lbl: "Électricité incluse", sub: "Charges électricité comprises" },
                      ].map((o) => (
                        <div key={o.key} className="flex items-center justify-between py-3 px-4 rounded-xl"
                          style={{ background: "#F8FAFC", border: "1px solid #F1F5F9" }}>
                          <div>
                            <div className="text-sm font-semibold" style={{ color: "#0F172A" }}>{o.lbl}</div>
                            <div className="text-xs" style={{ color: "#94A3B8" }}>{o.sub}</div>
                          </div>
                          <button className="toggle" onClick={() => set(o.key as keyof FormData, !form[o.key as keyof FormData])}
                            style={{ background: form[o.key as keyof FormData] ? "#2563EB" : "#E2E8F0" }}>
                            <span style={{
                              position: "absolute", top: "3px",
                              left: form[o.key as keyof FormData] ? "21px" : "3px",
                  ...
                  {/* Photos */}
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "#94A3B8" }}>Photos du bien</div>
                    <UploadFichier typeDocument="photo_bien" objetId={editBien?.id}
                      estPrincipal={true} label="Photo principale"
                ...
                <div className="sticky bottom-0 bg-white px-6 py-4 flex gap-3"
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    ...
                    {saving ? (
                      ...
                      <><IconCheck size={16} /> {editBien ? "Mettre à jour" : "Ajouter le bien"}</>
                    )}
                  </motion.button>
                </div>
              </motion.div>
  ...
  return (
    <Suspense fallback={
      <div className="flex h-screen items-center justify-center" style={{ background: "#F1F5F9" }}>
        <div style={{ width: "24px", height: "24px", borderRadius: "50%", border: "3px solid #E2E8F0", borderTopColor: "#2563EB", animation: "spin 0.7s linear infinite" }} />
      </div>
    }>
      <BiensPageContent />
    </Suspense>
